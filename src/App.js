...
const SingleSong = props => {
  ...
  const deleteSong = () => deleteTrack(id)
  ...
  return (
    <li>
      ...
      <div>
        <p>{duration}</p>
        <button type="button" onClick={deleteSong} testid="delete">
          Delete
        </button>
      </div>
    </li>
...
class App extends Component {
  state = {inputValue: '', songsList: initialTracksList}

  onChangeInput = event => this.setState({inputValue: event.target.value})

  deleteTrack = id => {
    const {songsList} = this.state
    const filteredSongs = songsList.filter(each => each.id !== id)
    this.setState({songsList: filteredSongs})
  }

  render() {
    const {inputValue, songsList} = this.state
